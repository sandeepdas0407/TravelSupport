...
- `backend/` — FastAPI app in `app/`, deployed via the Azure Functions ASGI wrapper in `function_app.py`. Each external API (Azure Maps, Open-Meteo, Google Places, Anthropic) has its own module under `app/services/`; `app/services/trip_planner.py` is the only place that orchestrates them.
...
- Never commit secrets. `ANTHROPIC_API_KEY`, `AZURE_MAPS_SUBSCRIPTION_KEY`, `GOOGLE_PLACES_API_KEY` are read from `.env` (backend) locally and from GitHub Actions secrets / Azure Static Web Apps app settings in CI/production.
